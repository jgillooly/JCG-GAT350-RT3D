...
namespace nc
{
	...
	void LightComponent::SetProgram(const res_t<Program> program, const std::string& name)
	{
		...
		if (castShadow) program->SetUniform("shadowVP", bias * GetShadowMatrix());
		if (castShadow) program->SetUniform("shadowBias", shadowBias);
	}
	...
	void LightComponent::ProcessGui()
	{
		...
		if (castShadow) ImGui::DragFloat("Shadow Bias", &shadowBias, 0.001f, 0, 0.5f);
	...
	glm::mat4 LightComponent::GetShadowMatrix()
	{
		...
		auto postition = m_owner->transform.position;
	...
	void LightComponent::Read(const nc::json_t& value)
	{
		// read json file
		int typei;
		READ_NAME_DATA(value, "lightType", typei);
		switch (typei) {
		case 0:
			type = Point;
			break;
